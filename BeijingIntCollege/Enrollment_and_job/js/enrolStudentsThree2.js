//评论
    ...
    function Trends_create(begin,index){
        if(criticism_json2.length!=0){
            for(var i=begin;(criticism_json2.length-begin>=5?i<5:i<criticism_json2.length);i++){
                var li = document.createElement('li');
                li.innerText = criticism_json2[i];
                criticism_ul.insertBefore(li,null);
                if(criticism_json2.length>=5?i==4:i==criticism_json2.length-1){
                    li.className = "bottom";
                }
            }
        }else{
        ...
        footer_nuber = Math.ceil(criticism_json2.length/5);
    ...
    publish.onclick = function(){
        ...
        Muitos_text.value = '';
        criticism_json2.unshift(content);
        alert("评论成功!");
        criticism_ul.innerHTML = '';
        Trends_create(0);
        }else{alert("评论内容不能为空!");}
